...
float getTemperature(){
    ...
    ESP_LOGI(TAG, "adc value: %d average: %.2f temperature: %.2f °C", value, average, ret);   
    return ret;
}
 ...
 */
float getPressure(){
    unsigned short value = i3AdcRead(ADC_CHANNEL_6);
    pressureQueue->push(value);
    float average = pressureQueue->average();
    //O PSI -> 0.5V rabaissé à 0.37V soit 425 en lecture ADC
    //145 PSI -> 4.5V rabaissé à 3.3V soit 4095 en lecture ADC
    float ret = i3LinearGetX(0, 425, 145, 4095, (int)average);
    if (ret < 0) ret = 0;
    ESP_LOGI(TAG, "adc value: %d average: %.2f pressure: %.2f PSI", value, average, ret);   
    return ret;
}
